...
var loadBackground = function() {
    ...
    images = preload({
            moon: 'img/moon2.jpg',
            scanlines: 'img/scanlines.png',
            vignette: 'img/vignette.png',
            dustNear: 'img/dust-near.jpg',
            dustFar: 'img/dust-far.jpg'
        }, function() {
            startTime = Date.now();
...
var draw = function() {
    ...
    // Aspect zoom
    var width = cw;
    var height = Math.ceil(images.moon.height / (images.moon.width / cw ));
    if( height < canvas.height ) {
        width = Math.ceil(images.moon.width / (images.moon.height / ch ));
        height = ch;
    }
    ...
    var scale = images.moon.width / width;

    // Moon
    ctx.globalAlpha = 1;
    ctx.fillStyle = '#111';
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    ctx.drawImage( images.moon, x, y, width, height );

    // Dust Layers
    var dx = x + i/5,
        dy = y + i/15 - $(document).scrollTop()/8; // extra vert scroll for dust layers
